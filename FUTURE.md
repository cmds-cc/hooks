...
- [ ] npm package support — `npx cc-hooks-install add spok-api` (resolve from npm in addition to GitHub)
- [ ] `--yes` flag — skip interactive prompt, install all default hooks
- [ ] `--project` flag — install to `.claude/settings.json` instead of global

## Site (`hooks.automate.builders`)
...
- [ ] Submit cc-hooks-install to cc-hook-registry for discoverability
